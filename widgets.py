...
class Paint(object):
    ...
    def __init__(self, master=None):
        ...
        self.pen_button = self.getBlackWhiteButton(text='Pen', command=self.use_pen)
        self.pen_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.brush_button = self.getBlackWhiteButton(text='Brush', command=self.use_brush)
        self.brush_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.color_button = self.getBlackWhiteButton(text='Choose a Color', command=self.choose_color)
        self.color_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.bg_button = self.getBlackWhiteButton(text='Choose a Background Color', command=self.choose_bg)
        self.bg_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.eraser_button = self.getBlackWhiteButton(text='Eraser', command=self.use_eraser)
        self.eraser_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.clear_button = self.getBlackWhiteButton(text='Clear', command=self.clear)
        self.clear_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.save_button = self.getBlackWhiteButton(text='Save Canva', command=self.savec)
        ...
        self.choose_size_button = self.getBlackWhiteSpinbox(from_=1, to=400, width=5, textvariable=self.var)
        self.choose_size_button.pack(side=LEFT, fill=BOTH, expand=1)

        self.setup()
        if master is None: self.root.mainloop()

    def getBlackWhiteButton(self, **kwargs):
        return Button(self.root, fg='black', bg='white', **kwargs)

    def getBlackWhiteSpinbox(self, **kwargs):
        return Spinbox(self.root, fg='black', bg='white', **kwargs)
    ...
    def paint(self, event):
        if not self.eraser_on:
            self.line_width = self.var.get() + 2*bool(self.active_button == self.brush_button)
        else:
            self.line_width = self.eraservar.get()
...
class TextLineNumbers(tk.Canvas):
    ...
    def redraw(self, *args):
        ...
        while True:
            ...
            if i.split('.')[0] == self.textwidget.index(INSERT).split('.')[0]:
                font = tkFont.Font(family=self.textwidget.family.get(), size=self.textwidget.size.get(), weight='bold')
            else:
...
class CustomText(Text, IPyText):
    def __init__(self, *args, **kwargs):
        ...
        self.family = StringVar(value='Quicksand')
...
def PanedText(master, **kwargs):
    text = CustomText(master, **kwargs)

    def configureOnSizeChanged(self, size):
        super.set(size)
    ...
    text.size.set = configureOnSizeChanged
    return text
...
def NotePad(root, buttons=False):
    if buttons:
        ...
        # Size
        img1 = ImageTk.PhotoImage(Image.open("res/IncreaseA.png").resize((50, 50)), master=frame_button)
        button = Button(frame_button, image=img1, compound="top", bg='white', command=OnBigger)
        button.image = img1
        button.pack(side=LEFT, fill=BOTH)
        
        img1 = ImageTk.PhotoImage(Image.open("res/DecreaseA.png").resize((50, 50)), master=frame_button)
        ...
        # Bold
        img1 = ImageTk.PhotoImage(Image.open("res/BoldB.ico").resize((50, 50)), master=frame_button)
        ...
        # Italic
        img2 = ImageTk.PhotoImage(Image.open("res/ItalicI.jpg").resize((50, 50)), master=frame_button)
        ...
        # Underline
        img3 = ImageTk.PhotoImage(Image.open("res/UnderU.png").resize((50, 50)), master=frame_button)
        ...
        # Strikethrough
        img4 = ImageTk.PhotoImage(Image.open("res/StrikeS.jpg").resize((50, 50)), master=frame_button)
        ...
        # Highlight
        img5 = ImageTk.PhotoImage(Image.open("res/highlight.jfif").resize((50, 50)), master=frame_button)
        ...
        # Color
        img6 = ImageTk.PhotoImage(Image.open("res/colorpal.png").resize((50, 50)), master=frame_button)
