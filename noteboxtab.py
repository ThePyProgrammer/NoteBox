...
class ImageCanvas(Paint):
    def __init__(self, image_file, nb):
        ...
        self.intangle = 0
        self.angle_button = Spinbox(self.root, from_=0, to=360, width=5, textvariable=self.angle, fg='black', bg='white', command=self.rotate)
...
def VisualHTnb(nb, filename=None): 
    ...
    def saveas(event=None):
        t = text.get("1.0", END)
        templocation = save(title="Select file to save as:", filetypes=(("HTML files", ".html"), ("Text files", ".txt"), ("All files","*.*")))
        if len(templocation):
    ...
    def new(event=None): VisualHTnb(nb, None)
...
def noteui(nb, filename=None):                                                #
    if filename is not None:
        if filename.split('.')[-1].lower() in ['jpg', 'jpeg', 'jfif', 'png', 'ico', 'bmp', 'eps', 'epsf', 'epsi', 'gif', 'dib', 'icns', 'im', 'jpe', 'jif', 'jfi', 'pcx', 'pbm', 'pgm', 'ppm', 'pnm', 'sgi', 'rgb', 'rgba', 'bw', 'int', 'inta', 'spi', 'tiff', 'tif', 'webp', 'xbm']:
    ...
    def func2(event=None):                                               #
        def execut():                                                    #
            excepto = ''                                                 #
            with stdoutIO() as s:                                        #
                try: exec(execute(text.get("1.0",END)))                  #
                except Exception as e: excepto = str(e)                  #
                                                                         #
            printf(s.getvalue())                                         #
        ...
        try: execut()                                                    #
        except Exception as e: msg.showerror('Error in Editor', str(e))  #
...
def new_func(nb, name=True, done=False):
    if name:
        ...
        while not isName(name):
            name = simpledialog.askstring('PyFunc.Error', f'Name your new function {name} again, the previous was not accepted. Do NOT add the .pyf extension.').strip()
...
def new_class(nb, name=True, done=False):
    if done:
        ...
        if not len(name): return 
        while not isName(name):
            name = simpledialog.askstring('PyClass.Error', f'Name your new class {name} again, the previous was not accepted. Do NOT add the .pycl extension.').strip()
    ...
    def func2(event=None):
        ...
        except Exception as e: msg.showerror(f'Error in Notebox.PyClass.{name}', str(e))
...
def notebox(filename=None):
    ...
    try:window.iconbitmap('NoteBoxIcon.ico')
    except:pass
    style = ttk.Style(window)
    style.theme_use('vista')

    if filename is None: savelocation = 'untitled'
    else: savelocation = filename
    ...
    def opennow(event=None):
        if event is None: openit()
        else: event.widget.event_generate("<Control-O>")
    ...
    def saveit(event=None):
        if savelocation == 'untitled':
            ...
            if len(templocation):
                ...
                MsgBox = msg.askyesnocancel('Exit Application','Are you sure you don\'t want to save the notebox?',icon = 'warning')
                if MsgBox is None: print('Escaped')
                elif MsgBox: window.destroy()
                else: saveit()
        else:
    ...
    def pause():
        try: mixer.music.pause()
        except: pass

    def stop():
        try: mixer.music.fadeout(600)
        except: pass
    ...
    def music(event=None):
        tempmusic = openfile(master=window, initialdir=fDir,title = "Select your cool music track",filetypes = (("mp3 Music Files","*.mp3"),("m4a Music Files","*.m4a")))
        if len(tempmusic):
    ...
    def yout(event=None): youtube(nb)

    def oth(event=None): othellonb(nb)

    def pia(event=None): pianonb(nb)
    ...
    def cut(event=None):
        if event is None: text.event_generate("<<Cut>>")
        else: event.widget.event_generate("<<Cut>>")

    def copy(event=None):
        if event is None: text.event_generate("<<Copy>>")
        else: event.widget.event_generate("<<Copy>>")

    def paste(event=None):
        if event is None: text.event_generate("<<Paste>>")
        else: event.widget.event_generate("<<Paste>>")

    def undo(event=None):
        if event is None: text.event_generate("<Control-z>")
        else: event.widget.event_generate("<Control-z>")

    def backs(event=None):
        if event is None: text.event_generate("<Backspace>")
        else: event.widget.event_generate("<BackSpace>")
...
fontmenu.add_command(label="{font}", command=Font{''.join(font.split())}())''', globals())
    ...
    if savelocation == 'untitled': nb.tab(root, text='NoteBox - untitled'.center(50))
    else: nb.tab(root, text=('Notebox - '+savelocation.split('/')[-1] + ' - ('+savelocation+')').center(50))
